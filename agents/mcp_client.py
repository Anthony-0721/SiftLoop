...
配置来源：
- 全局配置：`~/.siftloop/settings.json`
- 项目配置：`.siftloop/settings.json`
...
class McpConnection:
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "siftloop", "version": "1.0.0"},
        })
...
class McpManager:
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. 全局配置：~/.siftloop/settings.json
        global_path = Path.home() / ".siftloop" / "settings.json"
        self._merge_config_file(global_path, merged)

        # 2. 当前项目配置：<cwd>/.siftloop/settings.json
        project_path = Path.cwd() / ".siftloop" / "settings.json"
