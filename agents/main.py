...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="siftloop",
        description="SiftLoop — a minimal coding agent",
        add_help=False,
    )
...
def _resolve_api_config(cli_api_base: str | None) -> tuple[str | None, str | None, bool]:
    generic_api_key = _clean_env(os.environ.get("APIKEY")) or _clean_env(os.environ.get("SIFTLOOP_API_KEY"))
    openai_api_key = _clean_env(os.environ.get("OPENAI_API_KEY"))
    anthropic_api_key = _clean_env(os.environ.get("ANTHROPIC_API_KEY"))

    generic_api_base = _clean_env(os.environ.get("API")) or _clean_env(os.environ.get("SIFTLOOP_API_BASE"))
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .siftloop/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    ...
    if args.help:
        # 自定义帮助文本，展示 SiftLoop 支持的启动参数和 REPL 内置命令。
        print("""
Usage: siftloop [options] [prompt]
...
Options:
  ...
  --model, -m         Model to use (default: deepseek-chat, or SIFTLOOP_MODEL env, then MODEL env)
...
Examples:
  siftloop "fix the bug in src/app.ts"
  siftloop --yolo "run all tests and fix failures"
  siftloop --plan "how would you refactor this?"
  siftloop --max-cost 0.50 --max-turns 20 "implement feature X"
  SIFTLOOP_MODEL=deepseek-chat APIKEY=sk-xxx API=https://api.deepseek.com/anthropic siftloop "hello"
  SIFTLOOP_MODEL=gpt-4o OPENAI_API_KEY=sk-xxx OPENAI_BASE_URL=https://aihubmix.com/v1 siftloop "hello"
  siftloop --resume
  siftloop  # starts interactive REPL
""")
        sys.exit(0)

    # 将命令行布尔开关统一转换成 Agent 内部使用的权限模式。
    permission_mode = _resolve_permission_mode(args)
    # 模型优先使用命令行参数，其次读取 SIFTLOOP_MODEL，再读取 MODEL，最后回落到默认模型。
    model = args.model or os.environ.get("SIFTLOOP_MODEL") or os.environ.get("MODEL") or "deepseek-chat"
