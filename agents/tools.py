...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .siftloop/skills/. Returns the skill's resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".siftloop" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".siftloop" / "settings.json")
