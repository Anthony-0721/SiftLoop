...
本文档说明 SiftLoop 的在线 Skills 评测是怎么做的。重点解释每个阶段的名词含义、代码流程、输入输出和当前边界。
...
```text
~/.siftloop/skills/<skill>/SKILL.md
<project>/.siftloop/skills/<skill>/SKILL.md
```
...
```text
.siftloop/skill-evolution/online-eval/datasets/<lineage_id>/replay_pool.jsonl
```
...
```text
.siftloop/skill-evolution/online-eval/champions.json
.siftloop/skill-evolution/online-eval/champions/<lineage_id>/champion.json
.siftloop/skill-evolution/online-eval/champions/<lineage_id>/SKILL.md
```
...
```text
.siftloop/skill-evolution/online-eval/
...
```text
.siftloop/skill-evolution/online_eval_report.json
```
...
```text
Online skill eval:
  data_dir=/path/to/.siftloop/skill-evolution
...
```text
Online skill eval:
  data_dir=<project_root>/.siftloop/skill-evolution
  aggregate: ingests=8, ok_rate=100.0%, candidate_events=1, acceptance_rate=100.0%, replay_samples=1, rule_pass_rate=50.0%, llm=on, llm_rules=4, llm_judgments=1, llm_pass_rate=0.0%, candidates=1
  actions: none=7, add=1, merge=0, discard=0, failed=0, denied=0
  statuses: incubating=3, unobserved=1
  champion_statuses: incubating=3, unobserved=1
  ...
  report_file=<project_root>/.siftloop/skill-evolution/online_eval_report.json
```
...
```text
<project_root>/.siftloop/skill-evolution
```
...
```text
<project_root>/.siftloop/skill-evolution/online_eval_report.json
```
