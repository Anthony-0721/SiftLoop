...
SiftLoop Agent 是一个基于 Python 实现的 **自进化 Harness Agent**。它不是简单的命令行聊天工具，而是一个可运行、可阅读、可扩展的本地 Coding Agent Runtime：统一编排大模型推理、工具调用、文件编辑、Shell 执行、权限控制、长期记忆、Skills、自进化、MCP 外部工具、子 Agent 和会话恢复。

项目重点是 **Harness**：模型只负责推理和提出工具调用意图，真正的环境操作由 SiftLoop Runtime 统一做权限判断、工具执行、结果回写、上下文压缩和经验沉淀。它适合学习 Claude Code 类工具的底层机制，也适合作为个人 Coding Agent、项目分析助手或领域 Agent 的二次开发基础。
...
![SiftLoop Agent 总体架构](wiki/assets/architecture/01-overall-architecture.svg)
...
SIFTLOOP_MODEL=claude-sonnet-4-6
```
...
- `--model` 会覆盖 `.env` 中的 `SIFTLOOP_MODEL`（`MODEL` 仍作为旧名称兼容）。
...
SiftLoop 的核心特色是 **自进化 Skills**。它可以从用户明确反馈中抽取未来可复用的规则，并自动新增或合并到项目级或用户级 `SKILL.md`。

### 1. 开启自动自进化

默认 `SIFTLOOP_AUTO_SKILL_EVOLUTION` 是开启的。为了明确配置，建议在 `.env` 中写：

```env
SIFTLOOP_AUTO_SKILL_EVOLUTION=1
SIFTLOOP_AUTO_SKILL_TARGET=project
```

含义：

- `SIFTLOOP_AUTO_SKILL_EVOLUTION=1`：启用在线 Skill 自进化。
- `SIFTLOOP_AUTO_SKILL_TARGET=project`：自动新增的 Skill 写入当前项目 `.siftloop/skills/`。

如果希望沉淀为所有项目共享的个人 Skill：

```env
SIFTLOOP_AUTO_SKILL_TARGET=user
```

对应路径：

```text
project: <project>/.siftloop/skills/<skill_name>/SKILL.md
user:    ~/.siftloop/skills/<skill_name>/SKILL.md
```
...
```text
.siftloop/skill-evolution/usage.jsonl
.siftloop/skill-evolution/online_provenance.jsonl
.siftloop/skill-evolution/online_skill_provenance.json
.siftloop/skill-evolution/skill_usage_stats.json
.siftloop/skill-evolution/history/
.siftloop/skill-evolution/pruned/
```
...
```text
用户级：~/.siftloop/skills/<skill_name>/SKILL.md
项目级：<project>/.siftloop/skills/<skill_name>/SKILL.md
```
...
SiftLoop 支持 MCP 外部工具扩展。MCP Server 可以通过 stdio JSON-RPC 暴露工具，SiftLoop 会将其包装为 Agent 可调用工具。

配置来源：

```text
~/.siftloop/settings.json
<project>/.siftloop/settings.json
<project>/.mcp.json
```
...
docker run --rm -it \
  --env-file .env \
  -e SIFTLOOP_AUTO_SKILL_EVOLUTION=1 \
  -e SIFTLOOP_AUTO_SKILL_TARGET=project \
  -v "$PWD:/workspace" \
  -v siftloop-data:/root/.siftloop \
  siftloop --accept-edits
```
...
| 数据 | 路径 |
|------|------|
| 项目级 Skills | `.siftloop/skills/<skill_name>/SKILL.md` |
| 用户级 Skills | `~/.siftloop/skills/<skill_name>/SKILL.md` |
| Skills 自进化审计 | `.siftloop/skill-evolution/` |
| 长期记忆 | `~/.siftloop/projects/<project_hash>/memory/` |
| 会话历史 | `~/.siftloop/sessions/` |
| 大工具结果 | `~/.siftloop/tool-results/` |
| Plan Mode 计划 | `~/.siftloop/plans/` |
...
如果只看一个核心点：SiftLoop 是一个会把稳定用户反馈沉淀成 Skills 的 **自进化 Harness Agent**。
